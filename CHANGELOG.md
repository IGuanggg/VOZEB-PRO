...
- [Canvas] 修复画布基础操作链六项问题：节点提示词草稿持久化、剪贴板按本次系统内容分流、中文输入法组合态不再误提交、新建文字节点即可输入并区分编辑与拖动区域、撤销/重做归并当前未提交操作、图片导入即时占位并支持取消与单项重试。
- [Canvas] 加固异步上传与画布历史、复制/替换/取消/重试边界，修复成功后提示词面板状态，并补充浏览器回归。
- [安全与依赖] 升级 Next.js、Sharp、Tiptap 及测试/开发依赖，更新已修复的安全依赖版本；发布门禁依赖审计无已知漏洞。
- [发布] 将仓库、版本检查与容器镜像发布配置指向个人 GitHub 仓库，镜像按当前仓库所有者发布。
